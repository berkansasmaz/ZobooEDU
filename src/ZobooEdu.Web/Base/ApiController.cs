...
namespace ZobooEdu.Web
{
    ...
    public class ApiController : DbController //TODO: SecureDb controllerla değiştir
    {
        ...
        public UserManager<ZBUser> UserManager => _userManager ??
                                                  (UserManager<ZBUser>) HttpContext?.RequestServices.GetService(
                                                      typeof(UserManager<ZBUser>));
        ...
        public IActionResult Error(string message = default, string internalMessage = default, object data = default,
            int code = 400, List<ZBReturnError> errorMessage = null)
        {
            var rv = new ZBReturn
            {
                Success = false,
